...
# ╔═╡ 3de0c220-0d37-4795-9af7-f2f6a0ce9526
begin
using Javis, Animations, PlutoUI, Random, Colors, FileIO
import Luxor
end

# ╔═╡ 7c53c48a-8084-48f3-b8cd-9f346a1d736f
begin
Javis.Point
include("JUtils.jl")
using .JUtils
...
# ╔═╡ f2cc31ad-227c-4b25-b3c1-c43223339fc1
...
# ╔═╡ bf467584-2a71-461d-b000-b44ee8cdba93
let
n_frames = 100
height = 400
width = 500
left_margin = -200
process_animation = Video(width, height)
Background(1:n_frames, (args...)->(background("white"), sethue("black")))
p1, p2, p3, p4 = map(x->random_point(width÷2, height÷2), 1:4)
l1 = moving_text(frames=1:n_frames, start_point=p1, final_point=Point(left_margin, 100), body="B")

l1 = JUtils.moving_object(
		Object(
			1:100,
			(args...)->gen_text(body="B", pos=p1)
			),
		frames=1:n_frames, 
		start_point=p1, 
		final_point=Point(left_margin, 100), 
	)
	
l2 = moving_text(frames=1:n_frames, start_point=p2, final_point=Point(left_margin + 125, 100), body="4")
l3 = moving_text(frames=1:n_frames, start_point=p3, final_point=Point(left_margin + 250, 100), body="D")
l4 = moving_text(frames=1:n_frames, start_point=p4, final_point=Point(left_margin + 375, 100), body="S")

Object(1:n_frames, (args...) -> circle(Point(left_margin,100), 5, :fill))
Object(1:n_frames, (args...) -> circle(Point(left_margin + 125,100), 5, :fill))
Object(1:n_frames, (args...) -> circle(Point(left_margin + 250,100), 5, :fill))
Object(1:n_frames, (args...) -> circle(Point(left_margin + 375,100), 5, :fill))

pathofl1 = [p1]
Object(1:n_frames, 
	   (args...) -> continuous_lines!(pathofl1, pos(l1) + random_point(10,10), "red"), 
		keep=true
	)
	
pathofl2 = [p2]
Object(1:n_frames, 
	   (args...) -> continuous_lines!(pathofl2, pos(l2) + random_point(10,10), "red"), 
		keep=true
	)
	
pathofl3 = [p3]
Object(1:n_frames, 
	   (args...) -> continuous_lines!(pathofl3, pos(l3) + random_point(10,10), "red"), 
		keep=true
	)
pathofl4 = [p4]
Object(1:n_frames, 
	   (args...) -> continuous_lines!(pathofl4, pos(l4) + random_point(10,10), "red"), 
		keep=true
	)
	
render(
		process_animation, 
		pathname="process_animation.gif",
	)
end
...
git-tree-sha1 = "ea1f89a84c88b565cde4b707379e936b23744599"
